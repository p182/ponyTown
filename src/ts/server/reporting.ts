...
export const createReportSwears =
	(
		counter: Counter,
		reportSwearing: ReportAccount,
		timeoutAccount: TimeoutAccount,
		handlePromise = handlePromiseDefault,
	): OnMessageSettings =>
...
export const createReportForbidden =
	(counter: Counter, timeoutAccount: TimeoutAccount, handlePromise = handlePromiseDefault): OnMessageSettings =>
	(client, message, settings) => {
		const { accountId, account, reporter, shadowed } = client;
		...
		if (!mutedOrShadowed) {
			if (count >= limit) {
				...
				if (newAccount || settings.autoBanSwearing) {
					handlePromise(
						timeoutAccount(accountId, fromNow(duration)).then(() => reporter.system('Timed out for forbidden messages', msg)),
						reporter.error,
					);
				} else {
...
export const reportInviteLimit =
	(
		reportInviteLimitAccount: (account: string) => Promise<number>,
		message: string,
		handlePromise = handlePromiseDefault,
	): ReportInviteLimit =>
	({ accountId, reporter }) =>
		handlePromise(
			reportInviteLimitAccount(accountId).then(count => {
