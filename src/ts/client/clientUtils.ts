...
const otherValid = [
	...
	.join('')
	.split('')
	.reduce((set, c) => (set.add(c.charCodeAt(0)), set), new Set<number>());

export function isValid(c: number): boolean {
	return (
		(c >= 0x0020 && c <= 0x007e) || // latin
		...
		(c >= 0x2600 && c <= 0x26ff) ||
		(c >= 0x2700 && c <= 0x27bf) ||
		(c >= 0x2b00 && c <= 0x2bef) || // emoji
		(c >= 0x1f600 && c <= 0x1f64f) ||
		(c >= 0x1f680 && c <= 0x1f6f6) ||
		(c >= 0x1f300 && c <= 0x1f5ff) || // emoji
		(c >= 0x231a && c <= 0x231b) ||
		(c >= 0x23e9 && c <= 0x23fa) || // emoji
...
export function isValid2(c: number): boolean {
	return (
		(c >= 0x2b0 && c <= 0x2ff) || // Spacing Modifier Letters
		(c >= 0x531 && c <= 0x556) ||
		(c >= 0x559 && c <= 0x55f) ||
		(c >= 0x561 && c <= 0x587) ||
		(c >= 0x589 && c <= 0x58a) ||
		(c >= 0x58c && c <= 0x58f) || // Armenian
		(c >= 0x591 && c <= 0x5c7) ||
		(c >= 0x5d0 && c <= 0x5ea) ||
		(c >= 0x5f0 && c <= 0x5f4) || // Hebrew
		...
		(c >= 0x1f0c1 && c <= 0x1f0cf) ||
		(c >= 0x1f0d1 && c <= 0x1f0df) ||
		(c >= 0x1f0e0 && c <= 0x1f0f5) || // Playing Cards
...
export function registerServiceWorker(url: string, onUpdate: () => void) {
	try {
		if ('serviceWorker' in navigator && typeof navigator.serviceWorker.register === 'function') {
			...
			navigator.serviceWorker.register(url).then(worker => {
...
export function hardReload() {
	unregisterServiceWorker().then(() => location.reload(true));
}
