...
import { max, range, flatten, mapValues, dropRightWhile, isEqual, uniq, times, compact, includes, toPairs } from 'lodash';
import {
	ColorShadow,
	Nose,
	Eye,
...
function getEyesFromPsd({ objects2, sprites }: Result, eyesPsd: Psd, irisesPsd: Psd) {
	...
	const bases = [
		mergeCanvases(whites, lineart),
		...findLayerSafe('eyelashes', eyesPsd)
			.children.sort(compareLayers)
			.map(getCanvas)
	...
	const getEye = (get: ByIndexGetter) => (i: number) =>
		bases.map(base => {
...
function getMuzzlesFromPsd({ sprites, objects2 }: Result, psd: Psd) {
	...
	const expressionList = [0, 1, 24, 4, 9, 5, 2, 3, 8, 10, 11, 12, 13, 14, 15, 17, 18, 19, 20, 21, 22, 23, 30, 32, 6, 33];
	...
	objects2['noses: PonyNose[][][]'] = expressionList.map(expression =>
		muzzleIndices.map(type => {
...
function importSprites({ sprites, objects2 }: Result, sheet: Sheet) {
	...
	const sets: SetImportInfo[] = [...getSetNameLayer(sheet), ...(sheet.masks || [])];

	const maskFiles: any = {};

	if (sheet.masks) {
		compact(sheet.masks.map(m => m.maskFile)).forEach(file => (maskFiles[file] = openPsd(path.join(ponyPath, file + '.psd'))));
	}
	...
	const animations = sets.map(({ layerName, name, mask, reverse, maskFile, mirror, mirrorOffsetX }) => {
		...
		// [frame][type][pattern]
		const frames = trimRight(
			range(0, frameCount).map(frame => {
				return trimRight(
					range(0, typeCount).map(type => {
						...
						if (isCanvasEmpty(accessoryFrame)) {
							...
							const patternCanvases = patterns.map(getAndPad).map(pattern => clipPattern(accessoryFrame, pattern));

							return dropRightWhile(patternCanvases, isCanvasEmpty).map(
								patternFrame =>
									<ColorExtra>{
...
function getTreeStagesFromPsds(result: Result, psds: Psd[], name: string) {
	...
	psds.filter(psd => !isPalettePsd(psd)).forEach(psd => getTreesOrObjectFromPsd(result, psd, `${name}_${psd.name}`, palettes));
}
...
function addColorShadow(
	{ sprites, objects2 }: Result,
	name: string,
	color: ExtCanvas | undefined,
	shadow: ExtCanvas | undefined,
	palettes: number[][] | undefined,
) {
	objects2[name] = {
...
function createPalettes(psds: Psd[], layers: string[]) {
	...
	const otherPalettes = psds.filter(isPalettePsd).map(psd => {
...
function getObjectsFromPsds(result: Result, directory: string) {
	openPsdFiles(directory, /psd$/).forEach(psd => getObjectFromPsd(result, psd, psd.name));

	getDirectories(directory)
		.filter(dir => !/^_/.test(path.basename(dir)))
		.forEach(dir => {
			const files = openPsdFiles(dir, /psd$/).filter(psd => !/^_/.test(path.basename(psd.name)));
...
function obj(value: any, name: string, indent = false): string {
	if (value == null) {
		...
		if (/: StaticSprites(Extra)?$/.test(name)) {
			...
			if (/Extra$/.test(name)) {
				lines.push(
					`export const ${key}Extra: StaticSprites = [\n${types
						.map(patterns =>
							patterns
								? `\t[${patterns
										.map(x =>
											x!.extra ? `createColorPalette(${x!.extra}, [${addPalette(x!.palette!)}])` : 'emptyColorPalette()',
										)
										.join(', ')}],`
								: '\tundefined,',
						)
...
function createObj(s: ColorShadow & Nose & Eye & Tree & Animation & ColorExtra & TileSprites & Button & Emote, name: string) {
	if (s.frames && s.palette && s.shadow) {
		...
		return (
			`createButton(${s.border}, ${s.topLeft}, ${s.top}, ${s.topRight}, ${s.left}, ${s.bg},` +
...
function createFontCode(name: string, fontSprites: FontSprite[], sprites: string) {
	...
	return (
		`export const ${name} = createFont(${sprites}, [\n` +
...
function createSpritesTS(dest: string, config: SpriteTSConfig) {
	...
	ts = ts.replace(
		...
		[
			Object.keys(config.fonts)
				.map(key => createFontCode(key, config.fonts[key], 'sprites'))
				.join('\n\n'),
			Object.keys(config.fontsPal)
				.map(key => createFontCode(key, config.fontsPal[key], 'sprites2'))
				.join('\n\n'),
		].join('\n\n'),
	);

	ts += Object.keys(objects)
		.map(key => createObject(key, objects[key]))
		.join('');
	ts += Object.keys(objects2)
		.map(key => createObject(key, objects2[key]))
		.join('')
		.replace(/sprites/g, 'sprites2');
	...
	ts = ts.replace(
		'/*NAMED_PALETTES*/',
		toPairs(config.namedPalettes)
			.map(([key, value]) => `export const ${key} = palettes[${value}];`)
			.join('\n'),
	);
...
export function getFramesFromPSD({ sprites }: Result, psd: Psd, padY = 5): any[] {
	return findLayerSafe('frames', psd)
		.children.slice()
		.sort(compareLayers)
		.map(getCanvasSafe)
		.map(canvas => {
...
function createPonySprites(result: Result) {
	...
	sheets.filter(s => 'name' in s && !!s.file && !s.skipImport).forEach(s => importSprites(result, s as Sheet));
...
function createWalls(result: Result, rootPath: string) {
	createWall(result, 'wall_wood', openPsd(path.join(rootPath, 'wood.psd')), {
		...
		fullHeight: 85,
		halfHeight: 22,
		fullHeightVertical: 97,
		halfHeightVertical: 34,
	});
...
function createWall(result: Result, name: string, psd: Psd, config: WallConfig) {
	...
	result.objects2[`${name}_half`] = createSprites(
		half,
		config.fullHeight - config.halfHeight,
		config.halfHeight,
		config.halfHeightVertical,
	);
}
...
export function createSprites(log: boolean) {
	...
	const fontSprites = createFont(mainFont, 10, 10, canvas => addImage(result.images, canvas), { noChinese: true });
	const emojiSprites = createEmojis(mainEmoji, 10, 10, canvas => addImage(result.images, canvas));
	const smallFontSprites = createFont(tinyFont, 8, 9, canvas => addImage(result.images, canvas), { noChinese: true });
	const monoFontSprites = createFont(monoFont, 8, 9, canvas => addImage(result.images, canvas), {
		noChinese: true,
		mono: 4,
		onlyBase: true,
	});

	const fontSpritesPal = createFont(stripedMainFont, 10, 10, canvas =>
		addSprite(result.sprites, canvas, undefined, mainFontPalette),
	);
	const emojiSpritesPal = createEmojis(mainEmoji, 10, 10, canvas => addSprite(result.sprites, canvas, undefined, emojiPalette));
	const smallFontSpritesPal = createFont(
		stripedSmallFont,
		8,
		9,
		canvas => addSprite(result.sprites, canvas, undefined, smallFontPalette),
		{ noChinese: true },
	);
	const monoFontSpritesPal = createFont(
		stripedMonoFont,
		8,
		9,
		canvas => addSprite(result.sprites, canvas, undefined, smallFontPalette),
		{ noChinese: true, mono: 4, onlyBase: true },
	);
