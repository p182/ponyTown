...
function getShade(shade: number) {
	...
	if (shade > 130 && shade < 140) {
		// dark outline
		return 135;
	} else if (shade > 150 && shade < 165) {
		// outline
		return 159;
	} else if (shade > 165 && shade < 180) {
		// dark shade
		return 174;
	} else if (shade > 190 && shade < 210) {
		// shade
		return 204;
	} else if (shade > 210 && shade < 220) {
		// dark fill
		return 217;
	} else if (shade > 245) {
		// fill
...
export function imageToPalette(
	...
	for (let i = 0; i < length; i += 4) {
		...
		if (palette) {
			...
			if (index === -1) throw new Error(`Invalid color (pattern) [${pixel(pdata, i)}] (${x} ${y}) (${pat.info})`);
			if (alpha == null) throw new Error(`Invalid color [${pixel(data, i)}] (${x} ${y}) (${image.info})`);
