...
function generatePonyStats() {
	...
	iterate(
		db.Character.find({}, '_id name info'),
		doc => {
			try {
...
function fixCharacterStates() {
	...
	db.Character.countDocuments({ state: { $exists: true } }, (err, total) => {
		...
		iterate(
			db.Character.find({ state: { $exists: true } }, '_id name state'),
			doc => {
...
async function clearOrphanedIgnores() {
	...
	iterate(
		db.Account.find({}, '_id').lean(),
		doc => ids.add(doc._id.toString()),
		() => {
			iterate(
				db.Account.find({ ignores: { $exists: true, $not: { $size: 0 } } }, '_id ignores').lean(),
				doc => {
					const remove = [];
